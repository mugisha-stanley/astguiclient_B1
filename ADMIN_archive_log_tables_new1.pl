...
if (length($ARGV[0])>1)
	...
	if ($args =~ /--help/i)
        ...
        print "  [--days=XX] = number of days to archive past\n";
...
$del_time = "2017-08-18 10:56:34";

if ($daily > 0)
	{
	$CLImonths = '*** DAILY MODE ***';
	$del_epoch = ($secX - 432000);   # 24 hours ago
...
if ($days > 0)
	{
	$CLImonths = '*** DAYS MODE ***';
	$CLIdays_value=($CLIdays*86400);
	$del_epoch = ($secX - $CLIdays_value);   # days are converted to hours
	($RMsec,$RMmin,$RMhour,$RMmday,$RMmon,$RMyear,$RMwday,$RMyday,$RMisdst) = localtime($del_epoch);
	$RMyear = ($RMyear + 1900);
	$RMmon++;
	if ($RMmon < 10) {$RMmon = "0$RMmon";}
